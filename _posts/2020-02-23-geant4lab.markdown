---
layout: post
title:  "Intro to medical physics with Geant4 + Docker"
date:   2020-02-23
...
tags: [beginner, geant, particle physics, medical physics]
categories: beginner
terms: 1
---
...
We will now launch **our own simulations**. In order to achieve so, let us first **download the Docker image** that contains the necessary infrastructure. Remember, you can just click in the code cells to have them executed (one by one) in the terminal on the left side of this page:
...
By default, this last command assumes the input WRL file to have the name `simulation.wrl` and outputs an HTML file called, by default, `index.html`. You previous instruction is equivalent to:

```.term1
./wrl2html.py --wrl simulation.wrl --html index.html
```

 You can fly through this 3D HTML version by clicking [here](/){:data-term=".term1"}{:data-port="80"}. 
...
- **Particle count**. Available with the option `--count`. It is the number of particles within the simulated beam. The default value is 20. We suggest that you keep this value small. Otherwise you'll end up with a huge WRL file, very difficult to manage and visually analyse. 
...
```.term1
docker run -v ${PWD}:/root pammacdotnet/g4lpwd --type=e- --material=G4_Pb
./wrl2html.py
```

Again, you can specify a new WRL file (with the `--wrl` option). As you can see, 1 MeV electrons are unable to penetrate the material. On the contrary, some of them seem to *bounce back*. This is due to electrostatic repulsion against the electronic cloud covering this lead shield.
...
The previous simulation was obtained with the following command:

```.term1
docker run -v ${PWD}:/root pammacdotnet/g4lpwd --type=gamma --energy=10 --material=G4_Al
./wrl2html.py
```
...
```.term1
docker run -v ${PWD}:/root pammacdotnet/g4lpwd --type=gamma --energy=2 --count=100
./wrl2html.py
```

You'll see an scenario very similar to this one, where the photons, after traveling in straight lines and/or interacting with matter, seem to disappear:
...
<img src="https://66.media.tumblr.com/e1e51070e9065fb3cc74d24e8d59c2cf/tumblr_omq4gg2LUn1w4t58uo1_400.gifv" style="float:right;width:50%;padding:6px;">When this happens, Geant4 stops calculating any further interactions and any particle trace seems to suddenly vanish. However, even inside the simulation borders, some photons seem to *evaporate*. What's happening? The answer is simple: they are absorbed by the material. With more detail, these absorption processes correspond to the [**photoelectric effect**](https://en.wikipedia.org/wiki/Photoelectric_effect): photons are completely absorbed by electrons. 
...
```.term1
docker run -v ${PWD}:/root pammacdotnet/g4lpwd --type=e- --material=G4_Fe --count=10 --energy=50
./wrl2html.py
```

Each electron produces in turn an [***electromagnetic shower***](https://en.wikipedia.org/wiki/Particle_shower#Electromagnetic_showers). These showers consist of a bunch of photons that end up *photoelectrically absorbed*. 
...
```.term1
docker run -v ${PWD}:/root pammacdotnet/g4lpwd --type=gamma --material=G4_WATER --count=10 --energy=15
./wrl2html.py --wrl simulation.wrl --html index.html
```

You will see some events like the following, where a photon (with no mass and thus made of pure energy), after colliding with an atom, transfers all its momentum to the creation of two particles: an electron and anti-electron (positron). ![](https://tva1.sinaimg.cn/large/00831rSTgy1gcex54zkp3j31240bm3z0.jpg)

That is: energy (*E*) gets converted into matter (*m*), following the famous Einstein equation: *E=m·c·c*, where *c* is the speed of light. This process is known as ***pair production***. 



## Final words

Now it's time for you to play with your own simulations. This includes other materials, energies, particles, etc. As stated above, you can also increase the number of particles, but we suggest you keep this number below 100. This lab is based on the [article](https://iopscience.iop.org/article/10.1088/1361-6404/ab5011) *X-ray imaging virtual online laboratory for engineering undergraduates* (by Alberto Corbi, Daniel Burgos, Frank Vidal, Francisco Albiol and Alberto Albiol) and it has been carried out with success in the first semesters of the Computing Science degree at Universidad Internacional de La Rioja ([UNIR](https://www.unir.net)). 
